...
# === BEHAVIOR TREE LOGIC FOR WEREWOLF (IMPROVED) ===
class BehaviorNode:
    def run(self, game):
        pass
...
# Leaf: Attack the player directly
class AttackPlayer(BehaviorNode):
    def run(self, game):
        print(f"\nCAMPUS ANNOUNCEMENT: You were attacked by the werewolf!")
        game.player["stamina"] = 0
        return True

# Leaf: Fake alibi to confuse player
class FakeAlibi(BehaviorNode):
    def run(self, game):
        print("\nThe werewolf spreads a fake alibi to mislead you.")
        game.player["clues"] = max(0, game.player["clues"] - 1)
        return True

# Leaf: Manipulate high-social player
class Manipulate(BehaviorNode):
    def run(self, game):
        print("\nThe werewolf spreads rumors and confuses your investigation.")
        game.player["social"] = max(0, game.player["social"] - 15)
        return True

# Leaf: Just observe the player
class ObservePlayer(BehaviorNode):
    def run(self, game):
        print("\nThe werewolf is silently watching you...")
        return True

# Composite decision node: evaluate player as a target
class TargetPlayerNode(BehaviorNode):
    def run(self, game):
        if game.player["gpa"] > 3.5:
            if game.player["social"] > 60:
                return Manipulate().run(game)
            else:
                return AttackPlayer().run(game)
        else:
            if game.player["intelligence"] < 50:
                return FakeAlibi().run(game)
            else:
                return ObservePlayer().run(game)

# Inject into game
def build_werewolf_behavior(game):
    game.werewolf_behavior = SelectorNode([
        TargetPlayerNode(),
        SequenceNode([
            AttackSmartest(),
            LeaveClue()
...
class CollegeWerewolfGame:
    def generate_npc_reply(self, npc, context):
        ...
        if context == "intro":
            return "This semester feels different... like something's lurking beneath the surface."

        if context == "chat":
            if friendship > 60:
                return f"{npc['name']} grins: 'Always good to see you. Being {trait} helps me stay sane.'"
            elif friendship > 30:
                return f"{npc['name']} says: 'It's been alright... I guess I’m just doing the usual {trait} stuff.'"
            else:
                return f"{npc['name']} avoids eye contact: '...Yeah? What do you want?'"

        if context == "events":
            if suspicious >= 60:
                return f"{npc['name']} looks defensive: 'Stop asking so many questions!'"
            elif suspicious >= 30:
                return f"{npc['name']} mumbles: 'I heard something strange by the gym last night.'"
            else:
                return f"{npc['name']} shrugs: 'Dunno. Someone said there were noises near the dorm.'"
    ...
    def accuse_werewolf(self):
        ...
        # You can still vote even with 0 clues now. Proceed.
